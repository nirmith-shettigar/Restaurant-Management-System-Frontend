...
const makeStore = ({
  ...
  user = { id: 42, email: "customer@test.com", role: "CUSTOMER" },
} = {}) =>
  createStore({
    modules: {
      auth: {
...
describe("CustomerDashboard - booking submission", () => {
  it("calls createBooking with customerId when submitting a new booking", async () => {
    ...
    await wrapper
      .findComponent(BookTableModalStub)
      .vm.$emit("submit", bookingData);
    await flushPromises();

    expect(createSpy).toHaveBeenCalledWith(
      expect.objectContaining({ tableId: 2, customerId: 42 }),
    );
  });
});
