...
const mountDashboard = (storeOpts, router) =>
  mount(WaiterDashboard, {
    global: {
      ...
      stubs: {
        WaiterOrdersList: {
          name: "WaiterOrdersList",
          template: '<div class="waiter-orders-list" />',
        },
        TodayBookingsList: {
          name: "TodayBookingsList",
          template: '<div class="today-bookings-list" />',
        },
        ClipboardEdit: true,
        ClipboardList: true,
        CalendarDays: true,
...
describe("WaiterDashboard - view switch", () => {
  it("shows TodayBookingsList when Today's Bookings card is clicked", async () => {
    ...
    const bookingsCard = cards.find((c) =>
      c.text().includes("Today's Bookings"),
    );
