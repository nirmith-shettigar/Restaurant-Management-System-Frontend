...
describe("Create User Component", () => {
  ...
  it("renders form inputs", () => {
    ...
    expect(
      wrapper.find('input[placeholder="Enter email address"]').exists(),
    ).toBe(true);
    expect(wrapper.find('input[placeholder="Enter password"]').exists()).toBe(
      true,
    );
  });
  it("updates input values using v-model", async () => {
    const wrapper = mount(CreateUser);
    await wrapper
      .find('input[placeholder="Enter email address"]')
