<template>
  <div class="max-w-11/12 mx-auto p-6 font-sans">
        ...
        <div v-for="item in menuItems" :key="item.id"
          ...
          <div class="flex gap-4 flex-1">
            <div class="w-20 h-20 rounded-lg overflow-hidden bg-gray-100">
              <img :src="item.image" :alt="item.name" class="w-full h-full object-cover" @error="handleImageError">
            </div>

            <div class="flex-1">
              <h3 class="text-lg font-medium text-gray-800 mb-1">{{ item.name }}</h3>
              <p class="text-sm text-gray-600 mb-1 line-clamp-2">{{ item.description }}</p>
              <p class="text-base font-semibold text-green-600">₹{{ item.price }}</p>
            </div>
          </div>
